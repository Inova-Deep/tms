...
import type { ProfileStatusValue } from '@/types/api'

const auth = useAuthStore()

// Check if we have an employee selected
const hasEmployee = computed(() => !!auth.employeeId)

// Reactive employeeId - updates when sidebar switches employees
const employeeId = computed(() => auth.employeeId || '')

// Only fetch when we have an employeeId
...
// Profiles list for display
const profiles = computed(() => {
  if (!detail.value?.profile_status) return []
  return Object.entries(detail.value.profile_status).map(([profileId, status]) => ({
    profileId,
    status: status as ProfileStatusValue,
  }))
})
  ...
  <section class="page-section">
    ...
    <div class="page-header">
      <div class="page-header-left">
        <div class="title-row">
          <User class="icon-standard icon-muted" />
          <h1 class="page-title">My Training & Compliance</h1>
        </div>
        <div v-if="hasEmployee" class="header-info-row">
          <p class="text-heading-sm">{{ detail?.name ?? 'Loading...' }}</p>
          <Separator orientation="vertical" class="separator-sm" />
          <p class="page-meta">{{ employeeId }} · {{ detail?.worker_type ?? '' }}</p>
        </div>
      </div>
    </div>

    <!-- No Employee Selected State (Admin without employee switch) -->
    <div v-if="!hasEmployee" class="empty-state">
      <div class="empty-state-content">
        <div class="empty-state-icon">
          <Users class="empty-state-icon-inner" />
        </div>
        <h3 class="empty-state-title">No Employee Selected</h3>
        <p class="empty-state-description">
          As an admin, select an employee from the sidebar dropdown to view their training and compliance data.
        </p>
        <p class="page-meta mt-4">
          Click your avatar in the sidebar bottom-left → "Switch to Employee"
        </p>
      </div>
    </div>

    <!-- Dashboard Grid Layout (when employee is selected) -->
    <div v-else class="employee-detail-grid">
        ...
        <Card>
          ...
          <CardContent class="card-content-spaced">
            <div class="meta-list">
              ...
              <div class="meta-list-item">
                <Briefcase class="meta-list-icon" />
                <span>{{ detail?.worker_type ?? '—' }}</span>
              </div>
              <div class="meta-list-item">
                <CreditCard class="meta-list-icon" />
                <span>{{ detail?.cost_center ?? '—' }}</span>
              </div>
            </div>
            ...
            <div>
              ...
              <div class="badge-list">
                <Badge
                  v-for="p in profiles"
                  :key="p.profileId"
                  :variant="p.status === 'Eligible' ? 'success' : p.status === 'At Risk' ? 'warning' : 'destructive'"
                  class="badge-text-sm"
                >
                  {{ p.profileId }}
                </Badge>
                <p v-if="profiles.length === 0" class="page-meta">No profiles assigned.</p>
              </div>
            </div>
          </CardContent>
        </Card>
