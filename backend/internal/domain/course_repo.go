...
func (r *CourseRepository) GetByID(id string) (*Course, error) {
	...
	var code, description sql.NullString
	var createdAt, updatedAt sql.NullString
	err := r.DB.QueryRow("SELECT id, name, code, type, description, validity_months, created_at, updated_at FROM courses WHERE id = ?", id).
...
func (r *CourseRepository) GetByType(courseType string) ([]Course, error) {
	rows, err := r.DB.Query("SELECT id, name, code, type, description, validity_months, created_at, updated_at FROM courses WHERE type = ? ORDER BY name", courseType)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (r *CourseRepository) Update(course *Course) error {
	now := time.Now().Format(time.RFC3339)
	_, err := r.DB.Exec(`
		UPDATE courses SET name = ?, code = ?, type = ?, description = ?, validity_months = ?, updated_at = ? WHERE id = ?
...
func (r *CourseRepository) Delete(id string) error {
	var count int
	err := r.DB.QueryRow("SELECT COUNT(*) FROM requirements WHERE course_id = ?", id).Scan(&count)
	if err != nil {
		return err
	}
	if count > 0 {
	...
	err = r.DB.QueryRow("SELECT COUNT(*) FROM training_events WHERE course_id = ?", id).Scan(&count)
	if err != nil {
		return err
	}
	if count > 0 {
	...
	err = r.DB.QueryRow("SELECT COUNT(*) FROM evidence WHERE course_id = ?", id).Scan(&count)
	if err != nil {
		return err
	}
	if count > 0 {
	...
	err = r.DB.QueryRow("SELECT COUNT(*) FROM certifications WHERE course_id = ?", id).Scan(&count)
	if err != nil {
		return err
	}
	if count > 0 {
